function toggleTheme() {
    ...
    if (themeButton) themeButton.textContent = newTheme === 'dark' ? 'Világos mód' : 'Sötét mód';
}
...
function loadTheme() {
    ...
    if (themeButton) themeButton.textContent = savedTheme === 'dark' ? 'Világos mód' : 'Sötét mód';
}
...
function findFastestRacer() {
    ...
    rows.forEach(function(r) {
        ...
        const country = cols[1].textContent.trim();
        const years = cols[2].textContent.trim();
        const wins = parseInt(cols[3].textContent.trim()) || 0;
        const speed = parseFloat(cols[4].textContent.trim()) || 0;
        racers.push({name, country, years, wins, speed});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (quizSubmit) quizSubmit.addEventListener('click', checkQuizAnswers);
});